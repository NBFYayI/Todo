...
class TestTaskService:
    ...
    def test_list_user_tasks_success(self, db_session, created_multiple_tasks, created_user):
        """Test successfully listing user tasks."""
        from app.services.task import list_user_tasks
        
        tasks = list_user_tasks(db_session, created_user.id, skip=0, limit=100)
        
        assert len(tasks) == 3
        assert all(task.user_id == created_user.id for task in tasks)

    def test_list_user_tasks_no_tasks(self, db_session, created_user):
        """Test listing tasks when user has no tasks."""
        from app.services.task import list_user_tasks
        from fastapi import HTTPException
        
        with pytest.raises(HTTPException) as exc_info:
            list_user_tasks(db_session, created_user.id, skip=0, limit=100)
        
        assert exc_info.value.status_code == 404
        assert "No tasks found" in str(exc_info.value.detail)

    def test_get_existing_task_success(self, db_session, created_task, created_user):
        """Test successfully getting an existing task."""
        from app.services.task import get_existing_task
        
        task = get_existing_task(db_session, created_user.id, created_task.id)
        
        assert task.id == created_task.id
        assert task.user_id == created_user.id

    def test_get_existing_task_not_found(self, db_session, created_user):
        """Test getting a non-existent task."""
        from app.services.task import get_existing_task
        from fastapi import HTTPException
        
        with pytest.raises(HTTPException) as exc_info:
            get_existing_task(db_session, created_user.id, 99999)
        
        assert exc_info.value.status_code == 404
        assert "Task not found" in str(exc_info.value.detail)

    def test_get_existing_task_unauthorized(self, db_session, created_task):
        ...
        # Create another user
        other_user = create_user(db_session, UserCreate(email="other@test.com", password="password"))
        
        with pytest.raises(HTTPException) as exc_info:
            get_existing_task(db_session, other_user.id, created_task.id)
        
        assert exc_info.value.status_code == 403
        assert "Unauthorized access" in str(exc_info.value.detail)

    def test_make_task_success(self, db_session, created_user, sample_task_data):
        """Test successfully creating a task."""
        from app.services.task import make_task
        from app.schemas.task import TaskCreate
        
        task_data = TaskCreate(**sample_task_data)
        task = make_task(db_session, created_user.id, task_data)
    ...
    def test_change_task_success(self, db_session, created_task, created_user):
        ...
        updated_task = change_task(db_session, created_user.id, created_task.id, update_data)
        
        assert updated_task.title == "Updated Title"
        assert updated_task.completed is True
        assert updated_task.id == created_task.id

    def test_change_task_not_found(self, db_session, created_user):
        ...
        with pytest.raises(HTTPException) as exc_info:
            change_task(db_session, created_user.id, 99999, update_data)
        
        assert exc_info.value.status_code == 404
        assert "Task not found" in str(exc_info.value.detail)

    def test_change_task_unauthorized(self, db_session, created_task):
        ...
        with pytest.raises(HTTPException) as exc_info:
            change_task(db_session, other_user.id, created_task.id, update_data)
        
        assert exc_info.value.status_code == 403
        assert "Unauthorized access" in str(exc_info.value.detail)

    def test_remove_task_success(self, db_session, created_task, created_user):
        ...
        # Remove the task
        remove_task(db_session, created_user.id, task_id)
        
        # Verify it's deleted
        deleted_task = get_task(db_session, task_id)
        assert deleted_task is None

    def test_remove_task_not_found(self, db_session, created_user):
        """Test removing a non-existent task."""
        from app.services.task import remove_task
        from fastapi import HTTPException
        
        with pytest.raises(HTTPException) as exc_info:
            remove_task(db_session, created_user.id, 99999)
        
        assert exc_info.value.status_code == 404
        assert "Task not found" in str(exc_info.value.detail)

    def test_remove_task_unauthorized(self, db_session, created_task):
        ...
        with pytest.raises(HTTPException) as exc_info:
            remove_task(db_session, other_user.id, created_task.id)
